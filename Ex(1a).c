#include <stdio.h>
#define MAX_SIZE 100 // Maximum size of the array void createArray(int arr[], int *size);
void insertElement(int arr[], int *size, int element, int position); int searchElement(int arr[], int size, int element);
void deleteElement(int arr[], int *size, int position); void displayArray(int arr[], int size);
int main() {
int arr[MAX_SIZE]; int size = 0;
int choice, element, position, result; while(1) {
...
deleteElement(arr, &size, position); break;
case 5:
...
void insertElement(int arr[], int *size, int element, int position) { if(*size >= MAX_SIZE) {
printf("Error: Array is full. Cannot insert element.\n"); return;
}
if(position < 0 || position > *size) { printf("Error: Invalid position.\n"); return;
}
...
printf("Array elements: "); for(int i = 0; i < size; i++) { printf("%d ", arr[i]);
}
printf("\n");
}
